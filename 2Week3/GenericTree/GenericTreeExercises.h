...
auto leftnodeLC = left->addChild(16);
auto leftnodeRC = left->addChild(23);
leftnodeLC->addChild(42); 
}
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  std::vector<TreeNode *> rootChildPtrs;
  std::vector<TreeNode *> nextLevelChildPtrs;
  
  rootChildPtrs = rootNodePtr->childrenPtrs; // Level 1 nodes:  childs of root note
  
  while(rootChildPtrs.size()!=0){
    nextLevelChildPtrs.clear();
    
    for(size_t i=0; i<rootChildPtrs.size();i++){
      
      T data = rootChildPtrs[i]-> data;
      results.push_back(data);
      
      for(size_t j=0; j<rootChildPtrs[i]->childrenPtrs.size();j++){
      
      nextLevelChildPtrs.push_back(rootChildPtrs[i] -> childrenPtrs[j]);
    }
  }
  
  rootChildPtrs.clear();
  rootChildPtrs = nextLevelChildPtrs;
  }
  // ...
