...
int GridGraph::countEdges() const {
  ...
  // TODO: Your code here!
  // Visit each node 
  for (auto adjMap_i = adjacencyMap.begin(); adjMap_i != adjacencyMap.end(); adjMap_i++){
    auto edgePair = adjMap_i->second;
    // visit each edge of the node
    for(auto edge_i = edgePair.begin(); edge_i != edgePair.end() ; edge_i++){
      // count edges
      numEdges++;
    }
  }
  // for each a given node pair (a,b) same edge will be counted twice
...
void GridGraph::removePoint(const IntPair& p1) {
  ...
  for( auto neighbor_i = originalNeighbors.begin() ; neighbor_i != originalNeighbors.end() ; neighbor_i++ )
  {
    // remove edge 
    removeEdge( *neighbor_i, p1);
  }
...
std::list<IntPair> graphBFS(const IntPair& start, const IntPair& goal, const GridGraph& graph) {
  ...
  while (!exploreQ.empty() && !foundGoal && !tooManySteps) {
    ...
    GridGraph::NeighborSet neighbors; // Change this...
     // Get the current neighbor 2D points 
