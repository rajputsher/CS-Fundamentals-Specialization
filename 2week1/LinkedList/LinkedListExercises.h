...
if(!head_){
  ...
  Node *tempHead = head_; // Head pointing to the first element is stored to the temp location
  head_ = newNode; // Head should now point to the newNode which will be the first element
  tempHead->prev = newNode; // Previous first elements previous will be newNode
  newNode->next = tempHead; //newNodes next element will be previous first element
}
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
 ...
 LinkedList<T> extraElements = (!left.empty())? left: right;
 while(!extraElements.empty())
 {
   merged.pushBack(extraElements.front());
   extraElements.popFront();
 }
